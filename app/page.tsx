...
function titleFrom(raw: string) { const source = sourceFrom(raw); if (source !== "游戏信息归档") return `${source}游戏信息`; const line = raw.split("\n").find((part) => part.trim()) || raw; return line.replace(/^💫/, "").slice(0, 28); }
...
export default function Home() {
  ...
  const [showProgressInput, setShowProgressInput] = useState(false); const [progressUpdates, setProgressUpdates] = useState<Progress[]>([]); const [progressDate, setProgressDate] = useState("今日"); const [progressEnergy, setProgressEnergy] = useState(""); const [progressNote, setProgressNote] = useState(""); const [progressBreakthrough, setProgressBreakthrough] = useState(false);
  useEffect(() => { try { const saved = localStorage.getItem("blue-planet-incremental-intel"); if (saved) setEntries(JSON.parse(saved)); const savedProgress = localStorage.getItem("blue-planet-progress-updates"); if (savedProgress) setProgressUpdates(JSON.parse(savedProgress)); } catch {} }, []);
  ...
  const allProgress = useMemo(() => [...milestones.map((item) => ({ ...item, current: !progressUpdates.length && item.current })), ...progressUpdates.map((item, index) => ({ ...item, current: index === progressUpdates.length - 1 }))], [progressUpdates]);
  ...
  const addProgress = () => { const energy = Number(progressEnergy); if (!progressDate.trim() || !Number.isFinite(energy)) return; const fresh: Progress = { date: progressDate.trim(), energy, note: progressNote.trim() || "新增意识能量记录", breakthrough: progressBreakthrough ? "重大突破" : undefined, added: true, current: true }; const next = [...progressUpdates, fresh]; setProgressUpdates(next); localStorage.setItem("blue-planet-progress-updates", JSON.stringify(next)); setProgressEnergy(""); setProgressNote(""); setProgressBreakthrough(false); setShowProgressInput(false); setShowAllTimeline(false); };
  ...
  return <main>
    ...
    <section className="section intel" id="intel"><div className="section-heading intel-heading"><div><p className="eyebrow">INTELLIGENCE HUB</p><h2>游戏情报中心</h2><p>原文按空行分段，点击查看即可阅读完整内容。</p></div><div className="intel-actions"><button className="add-btn" onClick={() => setShowInput(!showInput)}><span>+</span> 归档新信息</button><label className="search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索原文、日期或来源…" />{query && <button onClick={() => setQuery("")} aria-label="清空搜索">×</button>}</label></div></div>
      ...
      <div className="intel-grid">{filtered.map((item) => <article className={`intel-card tone-${categories.findIndex((category) => category.name === item.category)} ${expanded === item.id ? "expanded" : ""}`} key={item.id}><div className="card-top"><span className="category-tag">{categories.find((category) => category.name === item.category)?.glyph} {item.category}</span>{item.added && <span className="priority">新增</span>}</div><p className="meta"><time>{item.date}</time><span>·</span><span>{item.source}</span></p><h3>{item.title}</h3><p className="raw-preview">{item.raw.replace(/\s*\n\s*/g, " · ")}</p><div className="details"><div className="original-label">原文全文 <span>本段保留 {item.raw.split("\n").length} 行</span></div>{item.raw.split("\n").map((paragraph, index) => <p key={index}>{paragraph}</p>)}</div><button className="expand" onClick={() => setExpanded(expanded === item.id ? null : item.id)} aria-expanded={expanded === item.id}>{expanded === item.id ? "收起原文" : "查看详细原文"}<span>{expanded === item.id ? "↑" : "→"}</span></button></article>)}{!filtered.length && <div className="empty"><OrbitMark /><h3>未检索到相关情报</h3><p>请尝试其他关键词或切换分类。</p></div>}</div>
    </section>
